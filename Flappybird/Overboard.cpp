...
void OverBoard::draw(QPainter *painter)
{
    ...
    painter->drawPixmap(this->overTextRect.x(), this->overTextRect.y(), this->overTextRect.width(), this->overTextRect.height(),this->pixmapList[10]);
    painter->drawPixmap(this->scoreLabelRect.x(),this->scoreLabelRect.y(), this->scoreLabelRect.width(), this->scoreLabelRect.height(), this->pixmapList[11]);
    ...
    {
        painter->drawPixmap(184.0,240.0,17.0,21.0, this->pixmapList[(this->score%1000)/100]);
        painter->drawPixmap(201.0,240.0,17.0,21.0,this->pixmapList[(this->score%100)/10]);
    }
    else if((this->score%100)/10 != 0)
    {
        painter->drawPixmap(201.0,240.0,17.0,21.0,this->pixmapList[(this->score%100)/10]);
    }
    painter->drawPixmap(218.0,240.0,17.0,21.0,this->pixmapList[this->score%10]);

    //最好成绩

    if((this->highestScore%1000)/100 != 0)
    {
        painter->drawPixmap(184.0,282.0,17.0,21.0,this->pixmapList[(this->highestScore%1000)/100]);
        painter->drawPixmap(201.0,282.0,17.0,21.0,this->pixmapList[(this->highestScore%100)/10]);
    }
    else if((this->highestScore%100)/10 != 0)
    {
        painter->drawPixmap(201.0,282.0,17.0,21.0,this->pixmapList[(this->highestScore%100)/10]);
    }
    painter->drawPixmap(218.0,282.0,17.0,21.0,this->pixmapList[this->highestScore%10]);

    //奖牌
    if(this->medal != NONE)
    {
        painter->drawPixmap(55.0,248.0,45.0,45.0, this->pixmapList[this->medal]);
    }
    //按钮
...
{
    int highestScore;
    if( !QFile::exists("./HighestScore.txt"))
    {
        QFile file;
        file.setFileName("./HighestScore.txt");
        file.open(QIODevice::WriteOnly | QIODevice::Text);
        QTextStream txtStrm(&file);
        txtStrm << this->score <<Qt::endl;
